...
import { 
  Search, Plus, Drumstick, ArrowUpDown, Loader2, Scale, Calendar, CheckCircle2, ThermometerSun, AlertCircle, ClipboardList, Activity, ChevronRight, ChevronDown, Users, User, MapPin
} from 'lucide-react';
...
export function Dashboard() {
  ...
  // Implemented local timezone string initialization to prevent midnight drift
  ...
  useEffect(() => {
    const channel = supabase.channel('animals-changes').on('postgres_changes', { event: '*', schema: 'public', table: 'animals' }, () => {
  ...
  const hierarchicalData = useMemo(() => {
    ...
    if (activeTab === 'ARCHIVED') baseData = allAnimals.filter(a => a.status === 'ARCHIVED');
    else if (activeTab !== 'ALL') baseData = allAnimals.filter(a => a.category === activeTab && a.status !== 'ARCHIVED');
    else baseData = allAnimals.filter(a => a.status !== 'ARCHIVED');
  ...
  const columns = useMemo(() => {
      ...
      cell: info => {
        ...
        return (
          <div className="flex items-center gap-3" style={{ paddingLeft: `${info.row.depth * 1.5}rem` }}>
            ...
            {avatarUrl ? <img src={avatarUrl} className="w-8 h-8 rounded-full object-cover shrink-0 border border-slate-200 shadow-sm" /> : <div className={`p-2 rounded-full shrink-0 shadow-sm ${isGroup ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>{isGroup ? <Users size={14} /> : <User size={14} />}</div>}
    ...
    if (EXOTIC_CATEGORIES.includes(activeTab)) {
      return [
        ...
        columnHelper.accessor('flying_weight', {
          header: 'Weight',
          cell: info => {
            const weight = info.getValue(); const unit = (info.row.original as any).weight_unit || 'g';
            if (!weight && info.row.original.record_type === 'GROUP') return <span className="text-[10px] text-slate-400 font-medium">N/A (Group)</span>;
            return <span className="text-sm font-bold text-slate-700 flex items-center gap-1.5"><Scale size={12} className="text-slate-400" />{weight ? `${weight}${unit}` : '--'}</span>;
          },
        }),
        columnHelper.display({ id: 'last_feed', header: 'Last Feed', cell: () => <span className="text-[10px] text-slate-400 italic font-medium uppercase tracking-widest">Pending Sync</span> }),
        columnHelper.accessor('next_feed_date', {
          ...
          cell: info => {
            ...
            return <span className="font-bold text-slate-800 text-xs uppercase tracking-tight flex items-center gap-1.5"><Drumstick size={12} className="text-amber-500" />{new Date(date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</span>;
          },
        }),
        columnHelper.accessor('target_day_temp_c', {
          header: 'Today\'s Temperature',
          cell: info => {
            const day = info.getValue(); const night = info.row.original.target_night_temp_c; const ambient = info.row.original.ambient_temp_only;
            if (!day && !night) return <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">--</span>;
            return (
              <div className="flex flex-col gap-0.5">
                <span className="text-xs font-bold text-orange-600 flex items-center gap-1"><ThermometerSun size={12} />{ambient ? 'Amb:' : 'Bask:'} {day}°C</span>
                {!ambient && night && <span className="text-[10px] text-blue-600 font-bold tracking-wide uppercase">Night: {night}°C</span>}
              </div>
            );
          },
        }),
    ...
    return [
      ...
      columnHelper.accessor('flying_weight', {
        ...
        cell: info => {
          const weight = info.getValue(); const unit = (info.row.original as any).weight_unit || 'g';
          if (!weight && info.row.original.record_type === 'GROUP') return <span className="text-[10px] text-slate-400 font-medium uppercase tracking-widest">N/A (Group)</span>;
          return <span className="text-sm font-bold text-slate-700 flex items-center gap-1.5"><Scale size={12} className="text-slate-400" />{weight ? `${weight}${unit}` : '--'}</span>;
        },
      }),
      columnHelper.display({ id: 'todays_feed', header: 'Today\'s Feed', cell: () => <span className="text-[10px] text-slate-400 italic font-medium uppercase tracking-widest">Pending Sync</span> }),
      columnHelper.display({ id: 'last_feed', header: 'Last Feed', cell: () => <span className="text-[10px] text-slate-400 italic font-medium uppercase tracking-widest">Pending Sync</span> }),
      locationColumn
    ];
  }, [activeTab]);
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
          <div className="relative w-full sm:w-auto">
            ...
            <input type="date" value={viewDate} onChange={e => setViewDate(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all shadow-sm" />
          </div>
          <div className="relative w-full sm:w-72">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" value={globalFilter ?? ''} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search entities, IDs, species..." className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all shadow-sm" />
          </div>
          <button onClick={() => setIsCreateAnimalModalOpen(true)} className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2 bg-emerald-600 hover:bg-emerald-50 text-white rounded-xl font-bold text-xs uppercase tracking-widest transition-all shadow-[0_0_15px_rgba(16,185,129,0.15)] shrink-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-48">
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400"><CheckCircle2 size={32} className="mb-3 opacity-20" /><p className="text-[10px] font-black uppercase tracking-widest">No pending tasks</p></div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-48">
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400"><AlertCircle size={32} className="mb-3 opacity-20" /><p className="text-[10px] font-black uppercase tracking-widest">No active medical alerts</p></div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white px-5 py-4 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-4"><div className="p-3 bg-emerald-50 text-emerald-600 rounded-xl border border-emerald-100"><Scale size={18} /></div><div><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Weighed Today</p><p className="text-xl font-black text-slate-900 leading-none mt-1.5">0 <span className="text-xs text-slate-400 font-bold uppercase tracking-widest ml-1">/ {totalInView}</span></p></div></div>
          </div>
          <div className="bg-white px-5 py-4 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-4"><div className="p-3 bg-amber-50 text-amber-600 rounded-xl border border-amber-100"><Drumstick size={18} /></div><div><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Fed Today</p><p className="text-xl font-black text-slate-900 leading-none mt-1.5">0 <span className="text-xs text-slate-400 font-bold uppercase tracking-widest ml-1">/ {totalInView}</span></p></div></div>
          </div>
        </div>
        <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2">
          {categories.map((category) => (
            <button key={category} onClick={() => setActiveTab(category)} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === category ? 'bg-emerald-50 text-emerald-600 border border-emerald-200 shadow-sm' : 'bg-white text-slate-500 border border-slate-200 hover:text-slate-700 hover:bg-slate-50'}`}>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
        {error ? (
          <div className="p-10 text-center text-rose-600 bg-rose-50 font-bold flex flex-col items-center gap-3"><AlertCircle size={24} /> Connection error: Verify RLS policies and DB connection.</div>
        ) : isLoading ? (
                ...
                {table.getHeaderGroups().map(headerGroup => (
                  <tr key={headerGroup.id}>
                    {headerGroup.headers.map(header => (
                      <th key={header.id} className="px-6 py-4 cursor-pointer hover:text-slate-700 transition-colors select-none group" onClick={header.column.getToggleSortingHandler()}>
                        <div className="flex items-center gap-2">{flexRender(header.column.columnDef.header, header.getContext())}<ArrowUpDown size={12} className={`opacity-0 group-hover:opacity-100 transition-opacity ${header.column.getIsSorted() ? 'opacity-100 text-emerald-500' : ''}`} /></div>
                      </th>
                    ))}
                  </tr>
                ))}
              ...
              <tbody className="divide-y divide-slate-100">
                {table.getRowModel().rows.length === 0 ? (
                  ...
                  table.getRowModel().rows.map(row => (
                    <tr key={row.id} className={`transition-colors group hover:bg-slate-50 ${row.original.record_type === 'GROUP' ? 'bg-slate-50/50' : 'bg-white'}`}>
                      {row.getVisibleCells().map(cell => <td key={cell.id} className="px-6 py-4 whitespace-nowrap">{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>)}
                    </tr>
                  ))
                )}
              </tbody>
