...
export function TimesheetsPage() {
  ...
  // Granular RBAC Permissions: Only Managers/Admins can see other people's timesheets
  const canViewAll = profile?.role === 'ADMIN' || profile?.role === 'MANAGER' || profile?.role === 'HR';
  
  const [selectedUserId, setSelectedUserId] = useState<string>('');

  // Default to the current logged-in user if they don't have access to view everyone
  useEffect(() => {
  ...
  const { data: timesheets = [], isLoading } = useQuery({
    queryKey: ['timesheets', selectedUserId],
    queryFn: () => timesheetService.getTimesheets(selectedUserId),
    enabled: !!selectedUserId, // Wait until the ID is set
  });
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-3">
            <Clock className="text-indigo-600" size={24} /> Timesheet Ledger
          </h1>
        ...
        <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl border border-slate-200 shadow-sm w-full md:w-auto">
          ...
          <select 
            ...
            disabled={!canViewAll} // Enforce RBAC
        ...
        <div className="w-full overflow-x-auto relative flex-1">
          ...
          <table className="w-full text-left min-w-[800px]">
            <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-200">
              <tr>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest w-1/5">Date</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest w-1/4">Staff Member</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Clock In</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Clock Out</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {timesheets.length === 0 && !isLoading ? (
                ...
                timesheets.map((record: any) => {
                  ...
                  return (
                    <tr key={record.id} className={`transition-colors ${isActive ? 'bg-indigo-50/30' : 'hover:bg-slate-50/60'}`}>
