...
// --- TIMEZONE FIX: Safely extract the user's LOCAL date string ---
...
const getDailyLogsOptions = (date: string) => queryOptions({
  ...
  queryFn: async () => {
    // --- TIMEZONE FIX: Calculate absolute local midnight and convert to UTC for Supabase ---
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
            ...
            <table className="w-full text-left text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-xs font-bold text-slate-700 bg-white">
                {filteredWorksheetRecords.length === 0 ? (
                  <tr><td colSpan={5} className="px-6 py-16 text-center text-slate-400 font-black uppercase tracking-widest bg-slate-50/30">No active records match the current section parameters.</td></tr>
                ) : (
                  <>
                    ...
                    {virtualItems.map((virtualRow) => {
                      ...
                      return (
                        <tr key={animal.id} ref={rowVirtualizer.measureElement} data-index={virtualRow.index} className="hover:bg-slate-50/40 transition-colors group">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <button type="button" onClick={() => triggerLogForm(animal, 'WEIGHT', log)} className={`w-full min-h-[46px] p-2 rounded-xl border border-dashed text-center flex flex-col justify-center items-center transition-all ${log?.weight_not_required ? 'bg-slate-100 border-slate-200 text-slate-400' : log?.weight_grams ? 'bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100 shadow-sm' : 'bg-slate-50 border-slate-200 text-slate-400 hover:bg-slate-100 hover:border-slate-300'}`}>
                              {log?.weight_not_required ? <span className="text-[9px] font-black uppercase tracking-widest">Exempt</span> : log?.weight_grams ? <><span className="text-sm font-black tracking-tight">{renderWorksheetWeight(log.weight_grams, animal.weight_unit)}</span><span className="text-[8px] text-slate-400 font-bold mt-0.5">Logged: {logTimeStr}</span></> : <><Scale size={14} className="opacity-40 mb-1" /><span className="text-[9px] font-black uppercase tracking-widest">Log Wt</span></>}
                            </button>
                          </td>

                          <td className="px-6 py-4 whitespace-nowrap">
                            <button type="button" onClick={() => triggerLogForm(animal, 'TEMPERATURE', log)} className={`w-full min-h-[46px] p-2 rounded-xl border border-dashed text-left transition-all flex flex-col justify-center ${log?.temperature_c || log?.basking_temp_c || log?.cool_temp_c ? 'bg-blue-50 border-blue-200 text-blue-800 hover:bg-blue-100 shadow-sm' : 'bg-slate-50 border-slate-200 text-slate-400 hover:bg-slate-100 hover:border-slate-300 items-center'}`}>
                          ...
                          <td className="px-6 py-4">
                            <div className="flex flex-col gap-2">
                              {meals.length > 0 && (
                                <div className="flex flex-col gap-1">
                                  {meals.map((meal: any, idx: number) => (
                                    <div key={idx} onClick={() => triggerLogForm(animal, 'FEEDING', log)} className="bg-amber-50/60 border border-amber-200/70 p-2 rounded-xl text-[10px] flex flex-col gap-0.5 shadow-sm cursor-pointer hover:bg-amber-100/50 transition-colors">
                              ...
                              <button type="button" onClick={() => triggerLogForm(animal, 'FEEDING', log)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 border border-slate-200 text-slate-500 hover:bg-slate-100 hover:text-amber-700 hover:border-amber-200 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all w-max shadow-sm"><Plus size={10} /> Add Feeding Event</button>
                            </div>
                          </td>

                          <td className="px-6 py-4 max-w-xs text-slate-500 font-medium leading-relaxed">
                            <button type="button" onClick={() => triggerLogForm(animal, 'OBSERVATION', log)} className="w-full text-left hover:bg-slate-100/50 p-2 rounded-xl transition-colors min-h-[44px] flex items-start">
                              <span className="text-[11px] leading-normal block">{log?.notes || <span className="text-slate-300 italic">No notes entered for this date...</span>}</span>
                            </button>
                          </td>
      ...
      {logModalState.isOpen && logModalState.animal && (
        <DailyLogFormModal
          isOpen={logModalState.isOpen} animal={logModalState.animal} mode={logModalState.mode} initialLogData={logModalState.initialData}
