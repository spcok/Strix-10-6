          ...
          <table className="w-full text-left min-w-[900px]">
            <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest w-1/4">Incident Classification</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest w-1/3">Operational Description</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">Status / Action</th>
              </tr>
            </thead>
                    ...
                    <tr key={inc.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1.5 items-start">
                          <p className="text-xs font-black text-slate-900 uppercase tracking-tight leading-tight">{inc.title}</p>
                        ...
                        {isOpen ? (
                          ...
                          <div className="flex flex-col items-end gap-1">
                            ...
                            {inc.resolution_notes && (
                              <span className="text-[9px] font-bold text-slate-400 w-48 truncate" title={inc.resolution_notes}>
                                {inc.resolution_notes}
                              </span>
                            )}
                          </div>
...
// ---------------------------------------------------------------------------
// COMPOUND INCIDENT MODAL COMPONENT
// ---------------------------------------------------------------------------
        ...
        <form id="incident-form" onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); form.handleSubmit(); }} className="p-6 space-y-6">
            ...
            <form.Field name="title">
              {(field) => (
                <div className="md:col-span-2">
                  ...
                  <input type="text" required value={field.state.value} onChange={e => field.handleChange(e.target.value)} placeholder="e.g. Sub-adult Eagle Owl Enclosure Breach" className={inputClass} />
                </div>
              )}
            </form.Field>
                  ...
                  <div className="flex bg-white border border-slate-200 rounded-xl p-1 shadow-sm flex-wrap gap-1">
                    {['ESCAPE', 'ANIMAL_BEHAVIOR', 'INFRASTRUCTURE', 'SLIP_TRIP_FALL', 'OTHER'].map((type) => (
                      <button 
                        ...
                        className={`flex-1 min-w-[120px] py-2.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${field.state.value === type ? 'bg-amber-500 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
                      >
                        {type.replace(/_/g, ' ')}
                      </button>
                    ))}
          ...
          <div className="space-y-5">
            <form.Field name="description">
              {(field) => (
                <div>
                  ...
                  <textarea required value={field.state.value} onChange={e => field.handleChange(e.target.value)} rows={3} className={`${inputClass} resize-none h-24`} placeholder="Detailed breakdown of how the breach occurred and exactly what failed..." />
                </div>
              )}
            </form.Field>
            ...
            <form.Field name="immediate_action_taken">
              {(field) => (
                <div>
                  <label className={`${labelClass} text-amber-700 flex items-center gap-1.5`}><AlertTriangle size={14} /> Immediate Corrective Actions Taken</label>
                  <textarea required value={field.state.value} onChange={e => field.handleChange(e.target.value)} rows={2} className={`${inputClass} resize-none h-20 bg-amber-50/30 border-amber-200 focus:border-amber-500`} placeholder="E.g., Perimeter locked down, secondary netting deployed over gap..." />
                </div>
              )}
            </form.Field>
          </div>

          {/* Section 2: Dynamic Clinical Escalation */}
            ...
            <form.Field name="requires_first_aid">
              {(field) => (
                <label className="flex items-center gap-3 p-4 bg-emerald-50 border border-emerald-200 rounded-xl cursor-pointer hover:bg-emerald-100 transition-colors shadow-sm">
                  <input type="checkbox" checked={field.state.value} onChange={e => field.handleChange(e.target.checked)} className="w-5 h-5 rounded border-emerald-300 text-emerald-600 focus:ring-emerald-500 bg-white" />
                  <div className="flex flex-col">
                    <span className="text-xs font-black text-emerald-800 uppercase tracking-widest flex items-center gap-2">
                      <BriefcaseMedical size={16} /> Human Injury Sustained
                    </span>
                    <span className="text-[10px] font-bold text-emerald-600 mt-0.5">Check this if the incident resulted in an injury requiring first aid or medical referral.</span>
                  </div>
                </label>
              )}
            </form.Field>
              ...
              {(needsFirstAid) => needsFirstAid && (
                <div className="mt-4 p-5 bg-white border-2 border-emerald-200 rounded-2xl shadow-sm space-y-5 animate-in fade-in slide-in-from-top-2">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest">Linked Clinical Report</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <form.Field name="person_type">
                      {(field) => (
                        <div>
                          ...
                          <select value={field.state.value} onChange={e => field.handleChange(e.target.value)} className={`${inputClass} border-emerald-200 focus:border-emerald-500`} required>
                    ...
                    <form.Field name="person_involved_name">
                      {(field) => (
                        <div>
                          ...
                          <input type="text" required value={field.state.value} onChange={e => field.handleChange(e.target.value)} placeholder="e.g. John Doe" className={`${inputClass} border-emerald-200 focus:border-emerald-500 focus:ring-emerald-500/20`} />
                        </div>
                      )}
                    </form.Field>
                    ...
                    <form.Field name="injury_description">
                      {(field) => (
                        <div className="md:col-span-2">
                          ...
                          <textarea required value={field.state.value} onChange={e => field.handleChange(e.target.value)} rows={2} className={`${inputClass} resize-none border-emerald-200 focus:border-emerald-500`} placeholder="E.g., Laceration on left index finger..." />
                        </div>
                      )}
                    </form.Field>
                    ...
                    <form.Field name="treatment_provided">
                      {(field) => (
                        <div className="md:col-span-2">
                          <label className={`${labelClass} flex items-center gap-2`}><Activity size={14} className="text-emerald-600" /> Treatment Administered</label>
                          <textarea required value={field.state.value} onChange={e => field.handleChange(e.target.value)} rows={2} className={`${inputClass} resize-none border-emerald-200 focus:border-emerald-500`} placeholder="E.g., Cleaned wound, applied plaster..." />
                        </div>
                      )}
                    </form.Field>
              ...
              <button 
                ...
                className={`px-8 py-2.5 flex items-center gap-2 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-lg disabled:opacity-50 ${
                  hasFirstAid 
                    ...
                    : 'bg-amber-600 hover:bg-amber-500 disabled:bg-amber-800 shadow-[0_0_15px_rgba(217,119,6,0.15)]'
                }`}
              >
...
function ResolutionModal({ incident, onClose, mutation }: { incident: any, onClose: () => void, mutation: any }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mutation.mutate({ id: incident.id, notes });
  };
